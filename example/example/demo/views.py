...
class ALUPayments(View):
    def get(self, request, *args, **kwargs):
        ...
        ipns = [
            ipn.pk for ipn in list(PayUIPN.objects.filter(REFNOEXT=order["ORDER_REF"]))
        ]
        ...
        return render(
            request,
            "choose_alu_token.html",
            {"orders": order["ORDER"], "details": details, "alu_tokens": alu_tokens},
        )
...
class TokenPayments(View):
    def get(self, request, *args, **kwargs):
        ipns = [
            ipn.pk
            for ipn in list(PayUIPN.objects.filter(REFNOEXT=DETAILS["ORDER_REF"]))
        ]
        ...
        return render(
            request,
            "choose_token.html",
            {"initial_order_ref": DETAILS["ORDER_REF"], "tokens": tokens},
        )
